...
public class PatientService {
    ...
    public List<PatientResponceDTO> getPatient() {
        ...
        List<PatientResponceDTO> dtos = patients.stream()
    ...
    public PatientResponceDTO createPatient(PatientRequestDTO patientRequestDTO) {
        if (patientRepository.existsByEmail(patientRequestDTO.getEmail())) {
            throw new EmailAlredyExistsException("A patient with this email" +
                    " alredy exists " + patientRequestDTO.getEmail());
        }
    ...
    public PatientResponceDTO updatePatient(UUID id,
                                            PatientRequestDTO patientRequestDTO) {

        // get pation
        Patient patient = patientRepository.
                findById(id).orElseThrow(()->
                        new PationNotFoundException
                                ("Pation not found with ID: "+ id ));
        // check email and etc
        if (patientRepository.existsByEmailAndIdNot(patientRequestDTO.
                getEmail(),id)) {
            throw new
                    EmailAlredyExistsException
                    ("A pation with this email" + " alredy exists " + patientRequestDTO.getEmail());
        }
